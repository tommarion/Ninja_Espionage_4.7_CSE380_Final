...
void Game::quitGame()
{
	...
	/*SpriteManager *sm = this->getGSM()->getSpriteManager();
	AnimatedSprite *player = sm->getPlayer();
	this->getGSM()->getPhysics()->removeCollidableObjectBot(player);*/


	// Iterate through bots and call removeCollidableObjectBot
	...
	{
		Bot *bot = (*botIterator);
		this->getGSM()->getPhysics()->removeCollidableObjectBot(bot);
		botIterator++;
	}*/
...
void Game::startGame()
{
	...
	if (!gsm->getSpriteManager()->getLoaded())
	{
	//	 THAT'S ONE THING YOU'LL BE DOING
		gsm->getSpriteManager()->setLoaded(true);
		dataLoader->loadWorld(this, currentLevelFileName);
	}
	else
	{
		dataLoader->reloadWorld(this, currentLevelFileName);
	}
}
