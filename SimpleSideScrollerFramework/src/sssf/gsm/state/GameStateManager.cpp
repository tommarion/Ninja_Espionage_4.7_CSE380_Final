...
void GameStateManager::goToGame()
{
	...
	mciSendString(LPCW_STOP_SPLASH, NULL, 0, NULL);
	mciSendString(LPCW_PLAY_GAME, NULL, 0, NULL);
}
...
void GameStateManager::goToMainMenu()
{
	...
	mciSendString(LPCW_STOP_GAME, NULL, 0, NULL);
	mciSendString(LPCW_PLAY_SPLASH, NULL, 0, NULL);
}

void GameStateManager::goToGameWon(Game * game)
{
	currentGameState = GS_GAME_WON;
	game->getGraphics()->setTextShouldBeRendered(false);
	PlaySound(LPCW_LEVEL_COMPLETE, NULL, SND_FILENAME | SND_ASYNC);
	game->getGSM()->getSpriteManager()->resetWon();
}

void GameStateManager::goToGameLost(Game *game)
{
	currentGameState = GS_GAME_LOST;
	game->getGraphics()->setTextShouldBeRendered(false);
	game->getGSM()->getSpriteManager()->resetDead();
}

void GameStateManager::goToGameFail(Game *game) // no more lives
{
	currentGameState = GS_GAME_FAIL;
	game->getGraphics()->setTextShouldBeRendered(false);
	game->getGSM()->getSpriteManager()->resetDead();
}
...
void GameStateManager::goToIntroScreen()
{
	currentGameState = GS_INTRO_SCREEN;
}
...
bool GameStateManager::isGameWon()
{
	return currentGameState == GS_GAME_WON;
}

bool GameStateManager::isGameLost()
{
	return currentGameState == GS_GAME_LOST;
}
