...
protected:
	...
	// USED TO DETERMINE THE DIRECTION FACING FO ITEMS
	unsigned int directionFacing;

	unsigned int i;

	unsigned int deadTimer;

	unsigned int currentLevel = 1; //sets Start Level to 1
...
public:
	...
	unsigned int		getDirectionFacing()	{ return directionFacing; }
	unsigned int		getDeadTimer()			{ return deadTimer; }
	...
	void setDirectionFacing(unsigned int initDirectionFacing);
	void updateSprite();
